...
├── scripts/
│   ├── compress_pdf.py                ← compress PDFs to 200 DPI (Ghostscript)
│   ├── install-hooks.sh               ← copies hooks into .git/hooks/ (run once)
│   └── hooks/
│       └── pre-commit                 ← auto-compresses staged PDFs before commit
├── assets/
│   ├── css/
...
│   │   └── slides.js                  ← PDF.js slide viewer (bilingual, zoom, drag)
│   └── images/
│       └── logo-om.svg
├── articles/
│   ├── 01/
│   │   ├── content.json               ← article data source
│   │   ├── index.html                 ← generated by build.js
│   │   ├── img/
│   │   │   ├── image-prompts.txt      ← AI image generation prompts
│   │   │   └── [infographic].png      ← optional raster infographic
│   │   └── slides/
│   │       ├── slides-mr.pdf          ← Marathi presentation (compress before commit)
│   │       └── slides-en.pdf          ← English presentation (compress before commit)
│   ├── 02/ … 10/                      ← same structure as 01
│   └── 11/ … 53/                      ← to be created
└── shared/
...
### Articles 01–10: content.json → build.js
All articles now use the data-driven build pipeline:
...
**Always use content.json for any new articles (11 onwards).**

---

## PDF Slideshow Workflow

Every article has a `slides/` folder for bilingual presentation PDFs.

### File naming
```
articles/NN/slides/slides-mr.pdf   ← Marathi presentation
articles/NN/slides/slides-en.pdf   ← English presentation
```

### Compress before pushing
**All PDFs must be compressed to 200 DPI before committing.**  
The pre-commit git hook does this automatically for any staged PDF.

```bash
# Manual compression (single file or glob)
python3 scripts/compress_pdf.py articles/NN/slides/slides-mr.pdf

# Compress all PDFs in the project at once
...
- Viewer is **hidden** if the PDF for the current language does not exist
- Viewer shows when the PDF file is present; no code change needed
- Language switch follows the global मराठी / English topbar toggle
- Zoom re-renders pages; 100% = full slide width fits the viewer
...
#### `slides` — PDF slide viewer (place before `phase2`)
...
Both `srcMr` and `srcEn` are optional. The viewer is hidden if the PDF file for the current language does not exist on the server. **Compress all PDFs to 200 DPI before committing** (`python3 scripts/compress_pdf.py <file>`).
...
### Step 4 — Add presentation PDFs (when ready)
```bash
mkdir -p articles/NN/slides
# Place PDFs here:
#   articles/NN/slides/slides-mr.pdf   ← Marathi
#   articles/NN/slides/slides-en.pdf   ← English

# Compress to 200 DPI before committing
python3 scripts/compress_pdf.py articles/NN/slides/slides-mr.pdf
python3 scripts/compress_pdf.py articles/NN/slides/slides-en.pdf
```
The `slides` block is already in every `content.json`. The viewer appears automatically once the PDF file exists. Alternatively, the pre-commit hook compresses staged PDFs automatically on `git commit`.

### Step 5 — Create image prompts (optional)
```bash
...
# Pre-commit hook auto-compresses any staged PDFs to 200 DPI
