...
All articles (01–53) use the same data-driven pipeline:
...
**Never hand-code article HTML directly.** Always edit `content.json` and rebuild.

Use `articles/01/content.json` through `articles/06/content.json` as reference examples.
...
| `IKS.setFontSize('md'|'lg'|'xl')` | Set body font size, persist to localStorage |
...
Article body text is controlled by the CSS variable `--art-fs` on `:root`. The three levels are:

| Level | Button | `--art-fs` | localStorage key |
|---|---|---|---|
| `md` (default) | A | 17.5px | `iks-fs` |
| `lg` | A+ | 20px | `iks-fs` |
| `xl` | A++ | 23px | `iks-fs` |

The font-size toggle (`A / A+ / A++`) is rendered inside the article header's `.art-meta` row, right-aligned. It only appears on article pages. Stored values of the old `sm` level are silently migrated to `md` on page load.

CSS: `.fs-toggle` and `.fs-btn` styles live in **`article.css`** (not `main.css`), because the toggle only appears on article pages against the dark `.art-header` background.
  ...
  "quadrants": [
    {
      ...
      "color": "green|gold|purple|saffron|gray",
      "headMr": "...", "headEn": "...",
      "subMr": "...", "subEn": "...",
...
Provide exactly 4 quadrants; each at a distinct `position`. The `gray` color (`#F1EFE8` / `#888780`) is used for neutral or "absence" states (e.g., deep sleep without active senses).
...
<div class="art-header">
  ...
  <div class="art-meta">
    ...
    <div class="fs-toggle">
      <button class="fs-btn active" data-fs-btn="md" onclick="IKS.setFontSize('md')">A</button>
      <button class="fs-btn" data-fs-btn="lg" onclick="IKS.setFontSize('lg')">A+</button>
      <button class="fs-btn" data-fs-btn="xl" onclick="IKS.setFontSize('xl')">A++</button>
    </div>
  </div>
</div>
```
...
When `nav.prev` is absent (first article), `build.js` automatically inserts `<div></div>` as a flex spacer so the next button stays right-aligned. You do not need to add this manually in `content.json`.
